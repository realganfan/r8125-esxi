...
rtl_ethtool_set_eee(struct net_device *net, struct ethtool_eee *eee)
{
		...
		//Disable for eee compablity issue
        tp->eee_adv_t = ethtool_adv_to_mmd_eee_adv_t(0); //eee->advertised);
...
rtl8125_hw_phy_config(struct net_device *dev)
{
        ...
        */
		...
		//Modified for eee compablity issue
		tp->eee_enabled = 0;
...
process_pkt:
                        ...
                        if (isMulticastType(skb->mhead)) //Port to Esxi
