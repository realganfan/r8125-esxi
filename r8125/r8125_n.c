...
struct sk_buff * r8125_skb_checksum_help(struct sk_buff *skb)
{
        int offset;
        unsigned int csum;
...
//The struct skbuff definition has no pkt_type member when __VMKLNX__ is defined, so we need to detail it.
...
rtl_ethtool_set_eee(struct net_device *net, struct ethtool_eee *eee)
{
        ...
        tp->eee_enabled = 0; //eee->eee_enabled;
                ...
                struct net_device *dev)
{
        ...
        if (skb->ip_summed == CHECKSUM_PARTIAL) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22) || defined(__VMKLNX__) //vmkernel has skb->nh union
                ...
                if (csum_cmd == 0) {
						...
						dprintk("checksum_partial proto=%x, dev->features=%x\n!", skb->protocol, dev->features);
#endif
                }
        }
...
//We need specific skb_checksum_help here for ESXi
#ifdef __VMKLNX__
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18) && !defined(__VMKLNX__)
...
process_pkt:
						...
						//ESXi skbuff has no pkt_type member
                        if ( isMulticastType(skb->mhead) )	//if (skb->pkt_type == PACKET_MULTICAST)
